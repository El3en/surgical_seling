...
<table  id="datatables" class="table no-margin col-md-12" style="border-bottom-color:red;padding-bottom:13px">
		...
		{
			$aa = mysqli_query($connect,("select si_product,image from products where p_id = '".$b['p_id']."'"));
			$bb = mysqli_fetch_array($aa);
			if($bb['si_product'] != NULL)
			{
	?>
<tr class="col-md-3">
	<td>
						<?php echo("<a href='single.php?p_id=".$b['p_id']."'>"); ?>
						 <div class="part-sec">					 
							 ...
							 <div class="part-info">
								 <?php echo("<a class='asd' href='single.php?p_id=".$b['p_id']."'>"); ?><h5><?php echo($bb['si_product']);?><span>$  <?php echo($b['price']);?></span></h5></a>
